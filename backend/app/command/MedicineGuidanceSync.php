...
use app\services\medicine\MedicineGuidanceService;
...
#[AsCommand('medicine:guidance-sync', '生成并保存常用药用药指导')]
class MedicineGuidanceSync extends Command
{
    protected function configure(): void
    {
        $this->addArgument('id', InputArgument::OPTIONAL, '指定常用药ID');
        $this->addOption('limit', null, InputOption::VALUE_OPTIONAL, '批量处理数量', 20);
        $this->addOption('force', null, InputOption::VALUE_NONE, '即使已有用药指导也强制重建');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $service = new MedicineGuidanceService();
        ...
        if (!$force) {
            $query->where(function ($builder) {
                $builder->whereNull('medication_guidance')
                    ->orWhere('medication_guidance', '');
            });
        }

        $medicines = $query->limit($limit)->get(['id']);
        if ($medicines->isEmpty()) {
            $output->writeln('<info>没有需要处理的药品记录</info>');
            return self::SUCCESS;
        }
        ...
        foreach ($medicines as $medicine) {
            try {
                ...
                if ($result['status'] === 'saved') {
                    $success++;
                    $output->writeln("<info>[成功]</info> ID {$medicine->id} 已保存用药指导");
                    continue;
                }
                $skipped++;
    ...
    private function handleOne(MedicineGuidanceService $service, int $id, bool $force, OutputInterface $output): int
    {
        try {
            $result = $service->generateAndSave($id, $force);
            $output->writeln("<info>ID {$id} {$result['message']}</info>");
            $output->writeln($result['medication_guidance']);
            return self::SUCCESS;
        } catch (\Throwable $e) {
