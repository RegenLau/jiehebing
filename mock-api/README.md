...
种子包含 28 名全新的、明确标识的后台建档模拟患者，均有出生日期和独立的 11 位模拟手机号并具备患者端登录资格；前 24 名关联 48 条用药及随访数据，后 4 名保留为空关联档案，用于验证空状态。另有当天/历史/未来计划、36 条不良反应、16 条常用药、13 篇文章、2 份问卷。保留部分近期建档患者用于未到填写日期状态，同时长期患者覆盖超过 30 天的未打卡计划；不会生成建档之前的计划。问卷 1 有 8 份模拟答卷，每份 6 题，问卷 2 为未作答草稿。问卷列表 `answerCount` 沿用后台 `COUNT(*)`，统计答案题目条数（初始 48），导出按答题患者每人一行。
