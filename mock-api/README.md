...
种子包含 28 名明显标识的模拟患者（24 名已建档、4 名未建档）、48 条关联用药、当天/历史/未来计划、36 条不良反应、16 条常用药、13 篇文章、2 份问卷。保留部分近期建档患者用于未到填写日期状态，同时长期患者覆盖超过 30 天的未打卡计划；不会生成建档之前的计划。问卷 1 有 8 份模拟答卷，每份 6 题，问卷 2 为未作答草稿。问卷列表 `answerCount` 沿用后台 `COUNT(*)`，统计答案题目条数（初始 48），导出按答题患者每人一行。
