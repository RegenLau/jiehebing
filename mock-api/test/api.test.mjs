...
async function start(t, now = NOW) {
  ...
  async function raw(
    path,
    { query, body, auth = token, method, headers = {} } = {},
  ) {
  ...
  async function login(username = "admin", password = PASSWORD) {
    ...
    const data = await ok(P + "login", {
      auth: "",
      body: { username, password, uuid: captcha.uuid, code: "1234" },
    });
  ...
  async function all(path, query = {}) {
    ...
    assert.equal(
      rows.length,
      total,
      `${path} returns every page without early truncation`,
    );
...
function workbookRows(buffer) {
  assert.equal(
    ...
    0x04034b50,
    "export is a ZIP based XLSX, not disguised JSON/CSV",
  );
  ...
  for (let index = 0; index < count; index++) {
    ...
    entries.set(
      name,
      (compression === 8 ? inflateRawSync(compressed) : compressed).toString(),
    );
  ...
  const textNodes = (xml) =>
    [...xml.matchAll(/<t(?:\s[^>]*)?>([\s\S]*?)<\/t>/g)]
      ...
      .join("");
  const shared = [
    ...(entries.get("xl/sharedStrings.xml") ?? "").matchAll(
      /<si(?:\s[^>]*)?>([\s\S]*?)<\/si>/g,
    ),
...
test("captcha, login failures, bearer authentication, and unknown routes are isolated", async (t) => {
  ...
  const badCaptcha = await api.json(P + "login", {
    body: {
      username: "admin",
      password: PASSWORD,
      uuid: captcha.uuid,
  ...
  for (const [username, password] of [
    ...
    assert.notEqual(
      (
        await api.json(P + "login", {
          body: { username, password, uuid: fresh.uuid, code: "1234" },
        })
      ).code,
      200,
    );
  ...
  assert.equal(
    (await api.json(P + "system/user", { auth: "invalid-token" })).code,
    401,
  );
  await api.login();
  ...
  assert.equal(
    (await api.json(P + "system/user", { auth: signedOutToken })).code,
    401,
  );
});
...
test("patient pages and medication records remain consistent with each assigned group scheme", async (t) => {
  ...
  const first = await api.ok(P + "patient/index", {
    query: { current: 1, size: 7 },
  });
  const second = await api.ok(P + "patient/index", {
    query: { current: 2, size: 7 },
  });
  ...
  assert.ok(
    first.list.every(
      (row) => !second.list.some((other) => row.id === other.id),
    ),
  );
  ...
  const patient = patients.find((row) =>
    plans.some((plan) => plan.user_id === row.id),
  );
  assert.ok(patient);
  assert.deepEqual(
    await api.ok(P + "patient/detail", { query: { user_id: patient.id } }),
    patient,
  );
  const medicines = await api.all(P + "patient/medicine-list", {
    user_id: patient.id,
  });
  assert.ok(medicines.length);
  const ownPlans = await api.all(P + "medication-plan/index", {
    user_id: patient.id,
    scope: "all",
  });
  assert.deepEqual(
    ids(ownPlans),
    ids(plans.filter((row) => row.user_id === patient.id)),
  );
  assert.ok(
    ownPlans.every(
      (row) =>
        row.patient_name === patient.name &&
        medicines.some((medicine) => medicine.id === row.medicine_id),
    ),
  );
  const ownReactions = await api.all(P + "adverse-reaction/index", {
    user_id: patient.id,
  });
  assert.deepEqual(
    ids(ownReactions),
    ids(reactions.filter((row) => row.user_id === patient.id)),
  );
  ...
  const groups = (
    await Promise.all(
      projects.map((project) =>
        api.ok(P + "project/detail", { query: { id: project.id } }),
      ),
    )
  ...
  for (const current of patients) {
    const group = groups.find(
      (row) =>
        row.id === current.group_id && row.project_id === current.project_id,
    );
    assert.ok(
      group?.medication,
      `patient ${current.id} belongs to a group with a medication scheme`,
    );
    assert.equal(current.medication_scheme_id, group.medication.id);
    assert.equal(
      current.medication_scheme_name,
      group.medication.snapshot.name,
    );
    const patientMedicines = await api.all(P + "patient/medicine-list", {
      user_id: current.id,
    });
    assert.deepEqual(
      patientMedicines
        .map((row) => row.common_medicine_id)
        .sort((a, b) => a - b),
      group.medication.snapshot.drugs
        .map((row) => row.drug_id)
        .sort((a, b) => a - b),
    );
    assert.ok(
      patientMedicines.every(
        (row) =>
          row.group_id === group.id &&
          row.medication_scheme_id === group.medication.id &&
          row.source === "group",
      ),
    );
    assert.ok(
      plans
        .filter((row) => row.user_id === current.id)
        .every(
          (row) =>
            row.group_id === group.id &&
            row.medication_scheme_id === group.medication.id,
...
test("medication and reaction filters include full history and precise overdue windows", async (t) => {
  ...
  for (const [query, predicate] of [
    ...
    [
      { plan_date: selected.plan_date, status: 0 },
      (row) => row.plan_date === selected.plan_date && row.status === 0,
    ],
    [
      { patient_name: selected.patient_name },
      (row) => row.patient_name.includes(selected.patient_name),
    ],
    [{ overdue: true }, (row) => row.plan_date < TODAY],
    [
      { overdue: true, status: 0 },
      (row) => row.plan_date < TODAY && row.status === 0,
    ],
    [
      { overdue: true, status: 0, overdue_range: "7d" },
      (row) =>
        row.plan_date >= "2026-09-02" &&
        row.plan_date < TODAY &&
        row.status === 0,
    ],
    [
      { overdue: true, status: 0, overdue_range: "30d" },
      (row) =>
        row.plan_date >= "2026-08-10" &&
        row.plan_date < TODAY &&
        row.status === 0,
    ],
  ]) {
    assert.deepEqual(
      ids(
        await api.all(P + "medication-plan/index", { scope: "all", ...query }),
      ),
      ids(plans.filter(predicate)),
    );
  }
  assert.ok(
    plans.some((row) => row.status === 0 && row.plan_date < "2026-08-10"),
  );
  assert.deepEqual(
    await api.all(P + "medication-plan/index", {
      scope: "all",
      patient_name: "不存在的虚构患者",
    }),
    [],
  );
  const reactions = await api.all(P + "adverse-reaction/index");
  for (const severity of [1, 2, 3]) {
    assert.deepEqual(
      ids(await api.all(P + "adverse-reaction/index", { severity })),
      ids(reactions.filter((row) => row.severity === severity)),
    );
  }
});
...
test("patient medication, reports and survey submissions follow a consistent enrollment timeline", async (t) => {
  ...
  const surveys = new Map(
    (await api.all(P + "survey/index")).map((survey) => [survey.id, survey]),
  );
  ...
  for (const patient of patients) {
    ...
    for (const medicine of medicines) {
      assert.ok(
        patient.enroll_date,
        `patient ${patient.id} has an enrollment date before receiving medicine`,
      );
      assert.ok(
        medicine.created_at.slice(0, 10) >= patient.enroll_date,
        `medicine ${medicine.id} starts no earlier than enrollment`,
      );
      assert.ok(
        medicine.created_at >= patient.created_at,
        `medicine ${medicine.id} is recorded after the patient exists`,
      );
      for (const plan of plans.filter(
        (row) => row.user_id === patient.id && row.medicine_id === medicine.id,
      )) {
        assert.ok(
          plan.plan_date >= medicine.created_at.slice(0, 10),
          `plan ${plan.id} follows its medicine start date`,
        );
        const expectedDay =
          (dayValue(plan.plan_date) - dayValue(medicine.created_at)) /
            86400000 +
          1;
        assert.equal(
          plan.day_number,
          expectedDay,
          `plan ${plan.id} day_number measures days since this medicine began`,
        );
        assert.ok(
          plan.created_at.slice(0, 10) >= patient.enroll_date,
          `plan ${plan.id} is not recorded before enrollment`,
        );
        assert.ok(
          plan.created_at >= medicine.created_at,
          `plan ${plan.id} is created after its medicine is recorded`,
        );
        const scheduledAt = `${plan.plan_date} ${plan.plan_time}:00`;
        assert.ok(
          scheduledAt >= plan.created_at,
          `plan ${plan.id} is created before its scheduled dose`,
        );
        if (plan.status === 1)
          assert.ok(
            plan.checked_at >= scheduledAt,
            `plan ${plan.id} is checked after its scheduled dose`,
          );
        medicationChecks++;
      }
    }
    for (const reaction of reactions.filter(
      ...
      assert.ok(
        reaction.occurred_at.slice(0, 10) >= patient.enroll_date,
        `reaction ${reaction.id} occurs after enrollment`,
      );
      assert.ok(
        reaction.occurred_at >= patient.created_at,
        `reaction ${reaction.id} does not precede its patient record`,
      );
      assert.ok(
        reaction.created_at >= reaction.occurred_at,
        `reaction ${reaction.id} is reported after it occurs`,
      );
      assert.ok(
        reaction.created_at.slice(0, 10) <= TODAY,
        `reaction ${reaction.id} is not reported in the future`,
      );
    }
    for (const status of statuses.filter((row) => row.answered)) {
      ...
      assert.ok(survey);
      assert.ok(
        survey.createdAt <= status.submitted_at,
        `survey ${survey.id} exists when patient ${patient.id} submits it`,
      );
      const expectedFillableDate = new Date(
        dayValue(patient.enroll_date) + survey.fillableDay * 86400000,
      )
        .toISOString()
        .slice(0, 10);
      assert.equal(status.fillable_date, expectedFillableDate);
      assert.ok(
        status.submitted_at.slice(0, 10) >= expectedFillableDate,
        `patient ${patient.id} submits no earlier than the questionnaire opens`,
      );
  ...
  assert.equal(
    medicationChecks,
    plans.length,
    "all plans belong to a known patient medicine and are checked",
  );
  assert.ok(
    medicationChecks > 0 && reactions.length > 0 && submissionChecks > 0,
    "timeline assertions cover nonempty data",
  );
});
...
test("dashboard metrics, trends, resources and todos can be recomputed from public detail APIs", async (t) => {
  ...
  for (const [field, endpoint] of [
    ["survey_total", "survey"],
    ["article_total", "health-article"],
    ["medicine_total", "common-medicine"],
  ]) {
    resourceCounts[field] = (await api.all(P + endpoint + "/index")).length;
  }
  for (const [range, startDate, days] of [
    ["today", TODAY, 1],
    ["7d", "2026-09-02", 7],
    ["30d", "2026-08-10", 30],
  ]) {
    ...
    const expected = plans.filter(
      (row) => row.plan_date >= startDate && row.plan_date <= TODAY,
    );
    const adverse = reactions.filter(
      (row) =>
        row.occurred_at.slice(0, 10) >= startDate &&
        row.occurred_at.slice(0, 10) <= TODAY,
    );
    ...
    assert.deepEqual(view.metrics, {
      ...
      completed_total: expected.filter((row) => row.status === 1).length,
    ...
    assert.equal(
      ...
      plans.filter(
        (row) =>
          row.status === 0 &&
          row.plan_date < TODAY &&
          (range === "today" || row.plan_date >= startDate),
      ).length,
    );
    assert.deepEqual(view.adverse_severity, {
      mild: adverse.filter((row) => row.severity === 1).length,
      moderate: adverse.filter((row) => row.severity === 2).length,
      severe: adverse.filter((row) => row.severity === 3).length,
    });
    ...
    for (let index = 0; index < days; index++) {
      const day = new Date(
        Date.parse(startDate + "T00:00:00Z") + index * 86400000,
      )
        .toISOString()
        .slice(0, 10);
      assert.equal(view.trend.labels[index], day.slice(5));
      assert.equal(
        view.trend.expected[index],
        expected.filter((row) => row.plan_date === day).length,
      );
      assert.equal(
        view.trend.completed[index],
        expected.filter((row) => row.plan_date === day && row.status === 1)
          .length,
      );
    }
  }
...
test("Shanghai date rolls over independently from UTC calendar date", async (t) => {
  ...
  await api.login();
  const view = await api.ok(P + "dashboard/overview", {
    query: { range: "today" },
  });
  assert.equal(view.date, TODAY);
...
test("historical dashboard overdue totals match as_of drilldowns for every range", async (t) => {
  ...
  for (const [range, firstDate] of [
    ...
    const query = {
      ...
      status: 0,
      overdue: true,
      ...(range === "today" ? {} : { overdue_range: range }),
    };
    const view = await api.ok(P + "dashboard/overview", {
      query: { range, date: historicalDate },
    });
    assert.equal(view.date, historicalDate);
    const expected = plans.filter(
      (plan) =>
        plan.status === 0 &&
        plan.plan_date < historicalDate &&
        (!firstDate || plan.plan_date >= firstDate),
    );
    const drilldown = await api.all(P + "medication-plan/index", {
      ...query,
      as_of: historicalDate,
    });
    assert.equal(
      view.todos.overdue_total,
      expected.length,
      `${range} dashboard respects its selected historical date`,
    );
    assert.equal(
      drilldown.length,
      view.todos.overdue_total,
      `${range} todo drilldown matches its dashboard count`,
    );
    assert.deepEqual(
      ids(drilldown),
      ids(expected),
      `${range} drilldown includes exactly the historical overdue records`,
    );
    assert.ok(
      drilldown.length > 0,
      `${range} is exercised with actual overdue records`,
    );
    const current = await api.all(P + "medication-plan/index", query);
    assert.notDeepEqual(
      ids(current),
      ids(drilldown),
      `${range} historical anchor changes the selected record set`,
    );
    for (const as_of of ["not-a-date", "2026-02-30"]) {
      assert.deepEqual(
        ids(await api.all(P + "medication-plan/index", { ...query, as_of })),
        ids(current),
        `invalid as_of=${as_of} uses today's established behavior`,
      );
    }
  }
});
...
test("administrator duplicate handling, profile, password updates and disabling enforce live sessions", async (t) => {
  ...
  const created = await api.ok(P + "admin/save", {
    body: {
  ...
  assert.equal(
    (
      await api.json(P + "admin/save", {
        body: { username: "test_operator", password: PASSWORD, status: 1 },
      })
    ).code,
    422,
  );
  assert.equal(
    (
      await api.json(P + "admin/update", {
        body: { ...created, username: "admin" },
      })
    ).code,
    422,
  );
  const operatorToken = await api.login("test_operator");
  await api.ok("/core/user/updateInfo", {
    body: {
      realname: "模拟测试员",
      phone: "10000000002",
  ...
  assert.notEqual(
    (
      await api.json("/core/user/modifyPassword", {
        body: {
          oldPassword: "wrong",
          newPassword: "Changed123456",
          confirmPassword: "Changed123456",
        },
      })
    ).code,
    200,
  ...
  await api.ok("/core/user/modifyPassword", {
    body: {
      oldPassword: PASSWORD,
      newPassword: "Changed123456",
      confirmPassword: "Changed123456",
  ...
  assert.equal(
    (await api.json(P + "system/user", { auth: operatorToken })).code,
    401,
  );
  ...
  await api.ok(P + "admin/update", {
    auth: rootToken,
    body: { ...created, status: 0 },
  });
  assert.equal(
    (await api.json(P + "system/user", { auth: changedToken })).code,
    401,
  );
  ...
  for (const endpoint of ["getLoginLogList", "getOperationLogList"]) {
    const page = await api.ok("/core/system/" + endpoint, {
      auth: rootToken,
...
test("health article edits, medicine status and cache clear persist within the same service", async (t) => {
  ...
  assert.ok(created.id);
  assert.equal(
    (await api.ok(P + "health-article/detail", { query: { id: created.id } }))
      .content,
    payload.content,
  );
  await api.ok(P + "health-article/save", {
    body: { ...payload, id: created.id, title: "接口验收科普文章已编辑" },
  });
  await api.ok(P + "health-article/toggle-status", {
    body: { id: created.id, status: 0 },
  });
  const matches = await api.all(P + "health-article/index", {
    keyword: "接口验收科普文章已编辑",
    status: 0,
  });
  assert.deepEqual(ids(matches), [created.id]);
  const medicine = (
    await api.all(P + "common-medicine/index", { status: 1 })
  )[0];
  assert.ok(medicine);
  await api.ok(P + "common-medicine/toggle-status", {
    body: { id: medicine.id, status: 0 },
  });
  assert.ok(
    (
      await api.all(P + "common-medicine/index", {
        status: 0,
        keyword: medicine.common_name,
      })
    ).some((row) => row.id === medicine.id),
  );
  assert.ok(
    !(await api.all(P + "common-medicine/index", { status: 1 })).some(
  ...
  assert.equal(
    (await api.json(P + "health-article/detail", { query: { id: 99999999 } }))
      .code,
    404,
  );
  assert.equal(
    (
      await api.json(P + "common-medicine/toggle-status", {
        body: { id: 99999999, status: 0 },
      })
    ).code,
    404,
  );
...
test("survey nested structure, per-patient answers, deletion protection and XLSX participant counts", async (t) => {
  ...
  const detail = await api.ok(P + "survey/detail", {
    query: { id: answered.id },
  });
  assert.equal(detail.questions.length, answered.questionCount);
  assert.deepEqual(
    new Set(detail.questions.map((row) => row.type)),
    new Set(["RADIO", "CHECKBOX", "TEXT"]),
  );
  ...
  assert.equal(
    answerRows,
    answered.answerRowCount,
    "answerRowCount counts question answer rows",
  );
  assert.equal(
    participants.length,
    answered.answerCount,
    "answerCount counts submitted questionnaires",
  );
  assert.equal(
    new Set(participants.map((row) => row.patient.id)).size,
    answered.participantCount,
  );
  assert.ok(participants.length > 0);
  const response = await api.raw(P + "survey/export", {
    query: { id: answered.id, current: 1, size: 1 },
  });
  assert.match(
      ...
      const exportSummary =
        answer.type === "TEXT"
          ...
          : answer.selected_options
              .map((option) => {
                const fields = option.trigger_input
                  ? option.input_fields
  ...
  await api.ok(P + "survey/toggle-status", {
    body: { id: empty.id, status: 0 },
  });
  assert.equal(
    (await api.ok(P + "survey/detail", { query: { id: empty.id } })).status,
    0,
  );
  await api.ok(P + "survey/delete", { body: { id: empty.id } });
  assert.equal(
    (await api.json(P + "survey/detail", { query: { id: empty.id } })).code,
    404,
  );
  const missing = await api.raw(P + "survey/export", {
    query: { id: 99999999 },
  });
...
test("adverse reaction XLSX includes project and group for all filtered records", async (t) => {
  ...
  const response = await api.raw(P + "adverse-reaction/export", {
    query: { severity: 2, current: 1, size: 1 },
  });
  assert.match(
    response.headers.get("content-type"),
    /application\/vnd.openxmlformats-officedocument.spreadsheetml.sheet/,
  ...
  for (const reaction of expected) {
    const row = rows.find(
      (values, index) => index > 0 && Number(values[0]) === reaction.id,
    );
  ...
  const emptyResponse = await api.raw(P + "adverse-reaction/export", {
    query: { patient_name: "不存在的虚构患者" },
  });
  assert.equal(
    workbookRows(Buffer.from(await emptyResponse.arrayBuffer())).length,
    1,
  );
});
...
test("multipart upload is byte accurate, visible in gallery, and restart resets resources, data and sessions", async (t) => {
  ...
  const gallery = await api.ok("/core/system/getResourceList", {
  ...
  assert.equal(
    (await api.all(P + "health-article/index")).length,
    articlesBefore.length + 1,
  );
  const restarted = await start(t);
  assert.equal(
    (await restarted.json(P + "system/user", { auth: token })).code,
    401,
  );
  assert.equal((await restarted.raw(filePath, { auth: "" })).status, 404);
  await restarted.login();
  assert.deepEqual(
    await restarted.all(P + "health-article/index"),
    articlesBefore,
  );
  const cleanGallery = await restarted.ok("/core/system/getResourceList", {
    query: { page: 1, limit: 100 },
  });
  assert.ok(
    !cleanGallery.data.some((resource) => resource.url === uploaded.url),
  );
  const badForm = new FormData();
  badForm.append("other", "missing file");
  assert.notEqual(
    (await api.json(P + "file/upload-file", { body: badForm })).code,
    200,
  );
});
...
test("research project statuses lock ended projects and their groups while restart resets", async (t) => {
  ...
  const followupMenu = (await api.ok(P + "system/menu")).find(
    (m) => m.path === "/followup",
  );
  assert.ok(
  ...
  assert.ok(
    patientMenu.children.some(
      (c) =>
        c.path === "management" &&
        c.name === "PatientManagement" &&
        c.meta.isHide,
    ),
  );
  const payload = {
  ...
  for (const invalid of [
    ...
    assert.notEqual(
      (await api.json(P + "project/save", { body: { ...payload, ...invalid } }))
        .code,
      200,
    );
  }
  const legacy = await api.ok(P + "project/detail", { query: { id: 1 } });
  const editedLegacy = await api.ok(P + "project/save", {
    body: {
      id: legacy.id,
      code: legacy.code,
      name: legacy.name,
...
test("basic group creation is independent from configuration and enforces project names", async (t) => {
  ...
  assert.equal(
    (
      await api.json(P + "project/group-create", {
        body: { project_id: 1, name: "A" },
      })
    ).code,
    401,
  );
  await api.login();
  const seeded = (await api.ok(P + "project/detail", { query: { id: 1 } }))
    .groups;
  assert.equal(seeded.length, 2);
  for (const body of [
    { project_id: 1, name: "" },
    { project_id: 999, name: "A" },
    { project_id: 1, name: "A", medication: { id: 1 } },
  ])
    assert.notEqual(
      (await api.json(P + "project/group-create", { body })).code,
      200,
    );
  const a = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "A" },
  });
  assert.equal(a.description, "");
  assert.equal(a.medication, null);
  assert.equal(a.reminder, null);
  assert.deepEqual(a.surveys, []);
  assert.deepEqual(a.tasks, []);
  assert.deepEqual(a.participant_ids, []);
  assert.equal(a.articles, undefined);
  assert.equal(a.contact_ids, undefined);
  assert.notEqual(
    (
      await api.json(P + "project/group-create", {
        body: { project_id: 1, name: "a" },
  ...
  await api.ok(P + "project/group-create", {
    body: { project_id: 2, name: "A" },
  });
  assert.equal(
    (
      await api.json(P + "project/group-detail", {
        query: { project_id: 2, id: a.id },
      })
    ).code,
    404,
  );
  ...
  await restarted.login();
  assert.deepEqual(
    (await restarted.ok(P + "project/detail", { query: { id: 1 } })).groups,
    seeded,
  );
});
...
test("group basic edit preserves configuration and returns current enrolled patient data", async (t) => {
  ...
  const configured = await api.ok(P + "project/group-detail", {
    query: { project_id: a.project_id, id: a.id },
  });
  const edited = await api.ok(P + "project/group-basic-save", {
    body: {
      id: a.id,
  ...
  assert.equal(edited.name, "基础编辑A组（新）");
  assert.equal(edited.description, "只修改基础信息");
  assert.deepEqual(edited.medication, configured.medication);
  assert.deepEqual(edited.reminder, configured.reminder);
  assert.deepEqual(edited.surveys, configured.surveys);
  assert.deepEqual(edited.tasks, configured.tasks);
  assert.deepEqual(edited.participant_ids, configured.participant_ids);
  const patient = patients.find((p) => p.id === configured.participant_ids[0]);
  assert.deepEqual(edited.participants[0], {
    id: patient.id,
    ...
    study_state: patient.study_state,
  });
  assert.deepEqual(
    await api.ok(P + "project/group-detail", {
      query: { project_id: a.project_id, id: a.id },
    }),
    edited,
  );
  const unchanged = await api.ok(P + "project/group-basic-save", {
    body: {
      id: a.id,
      project_id: a.project_id,
  ...
  for (const body of [
    ...
    assert.notEqual(
      (await api.json(P + "project/group-basic-save", { body })).code,
      200,
    );
  assert.equal(
    (
      await api.json(P + "project/group-basic-save", {
        body: {
          id: a.id,
...
test("seed research groups cover every patient exactly once", async (t) => {
  ...
  assert.deepEqual(
    ...
    patients.map((patient) => patient.id).sort((a, b) => a - b),
  );
  for (const patient of patients) {
    ...
    assert.ok(
      group.participants.some(
        (item) => item.id === patient.id && item.mobile === patient.mobile,
      ),
    );
  }
});
...
test("saved group configuration validates sources and members without changing patient execution", async (t) => {
  ...
  const patients = await api.all(P + "patient/index");
  const initialProjectPatientCount = (await api.all(P + "project/index")).find(
    (p) => p.id === 1,
  ).patient_count;
  const catalog = await api.ok(P + "project/catalog");
  assert.equal(catalog.articles, undefined);
  ...
  assert.deepEqual(saved.participant_ids, [1, 2]);
  assert.equal(
    saved.participants[0].name,
    patients.find((p) => p.id === 1).name,
  );
  assert.equal(
    (await api.all(P + "project/index")).find((p) => p.id === 1).patient_count,
    initialProjectPatientCount + 2,
  );
  ...
  assert.notEqual(
    (
      await api.json(P + "project/group-save", {
        body: { ...b, participant_ids: [1] },
      })
    ).code,
    200,
  );
  await api.ok(P + "project/group-save", {
    body: { ...other, participant_ids: [1] },
  });
  for (const invalid of [
    { participant_ids: [999999] },
    { participant_ids: [1, 1] },
    { participant_ids: [1.2] },
    { surveys: [schedule(9999)] },
    { surveys: [schedule(survey.id), schedule(survey.id)] },
    ...
    { medication: { ...config.medication, quantities: [] } },
    {
      medication: {
        ...config.medication,
        id: catalog.medication_schemes.find((s) => s.status === 0).id,
      },
    },
    { reminder: { id: 9999 } },
    {
      reminder: { id: catalog.reminder_schemes.find((s) => s.status === 0).id },
    },
    { article_ids: [1] },
    { contact_ids: [1] },
  ])
  ...
  assert.equal(
    (
      await api.json(P + "project/group-save", {
        body: { ...saved, project_id: 2 },
      })
    ).code,
    404,
  );
  assert.notEqual(
    (await api.json(P + "project/group-save", { body: config })).code,
    200,
  );
  await api.ok(P + "survey/toggle-status", {
    body: { id: survey.id, status: 0 },
  });
  const updated = await api.ok(P + "project/group-save", {
    body: { ...saved, description: "保留原模板内容", participant_ids: [2] },
  });
  assert.deepEqual(updated.surveys[0].snapshot, saved.surveys[0].snapshot);
  assert.notEqual(
    (
      await api.json(P + "project/group-save", {
        body: { ...b, surveys: [schedule(survey.id)] },
      })
    ).code,
    200,
  );
  ...
  const choices = await api.ok(P + "project/participants", {
    query: { project_id: 1 },
  });
  assert.equal(choices.find((p) => p.id === 1).group_id, b.id);
  await api.ok(P + "survey/toggle-status", { body: { id: 2, status: 1 } });
  await api.ok(P + "project/group-save", {
    body: { ...updated, surveys: [schedule(2)] },
  });
  assert.match(
    (await api.json(P + "survey/delete", { body: { id: 2 } })).message,
    /研究分组引用/,
  );
...
test("medication schemes maintain versions, validate drugs, and preserve group snapshots", async (t) => {
  const api = await start(t);
  await api.login();
  const data = await api.ok(P + "medication-scheme/detail", {
    query: { id: 1 },
  });
  const body = {
    ...data,
    ...
    pickup_days: 20,
    advance_days: 5,
    drugs: data.drugs.map((d) => ({ ...d, quantity: 40 })),
  };
  const row = await api.ok(P + "medication-scheme/save", { body });
  assert.equal(row.treatment_days, 60);
  assert.equal(row.history.length, 1);
  assert.notEqual(
    (await api.json(P + "medication-scheme/save", { body })).code,
    200,
  );
  for (const change of [
    { drugs: [] },
    { advance_days: 21 },
    { drugs: [body.drugs[0], body.drugs[0]] },
    { drugs: [{ ...body.drugs[0], times: "25:00" }] },
    { drugs: [{ ...body.drugs[0], dose: "0" }] },
  ])
    assert.notEqual(
      (
        await api.json(P + "medication-scheme/save", {
          body: { ...body, name: "无效方案", ...change },
        })
      ).code,
      200,
    );
  const group = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "方案隔离组" },
  });
  const configured = await api.ok(P + "project/group-save", {
    body: {
      ...group,
  ...
  const changed = await api.ok(P + "medication-scheme/save", {
    body: { ...row, name: "新版本方案", reason: "更新名称" },
  });
  assert.equal(changed.history.length, 2);
  assert.notEqual(changed.version, row.version);
  assert.notEqual(
    (
      await api.json(P + "medication-scheme/save", {
        body: { ...row, reason: "旧版本" },
  ...
  const after = await api.ok(P + "project/group-detail", {
    query: { project_id: 1, id: group.id },
  });
  assert.deepEqual(after.medication, configured.medication);
  const disabled = await api.ok(P + "medication-scheme/status", {
    body: {
      id: row.id,
  ...
  const catalog = await api.ok(P + "project/catalog");
  assert.equal(
    catalog.medication_schemes.find((s) => s.id === row.id).status,
    0,
  );
});
...
test("task templates support maintenance, filters, revision checks and isolated references", async (t) => {
  ...
  const after = await api.ok(P + "project/group-detail", {
    query: { project_id: 1, id: g.id },
  });
  assert.deepEqual(after.tasks, configured.tasks);
  const disabled = await api.ok(P + "task-template/status", {
    body: {
      id: row.id,
      version: changed.version,
      status: 0,
  ...
  assert.equal(disabled.status, 0);
  const filtered = await api.ok(P + "task-template/index", {
    query: { type: "检查", status: 0, keyword: "复查模板测试" },
  });
...
test("legacy reminder schemes do not override the reminder time configured on each task", async (t) => {
  ...
  for (const invalid of [
    ...
    { ...body, name: "错误时间", task_remind_time: "25:00" },
    { ...body, name: "错误提前量", pickup_advance_days: 61 },
  ])
    assert.notEqual(
      (await api.json(P + "reminder-scheme/save", { body: invalid })).code,
      200,
    );
  const group = await api.ok(P + "project/group-create", {
  ...
  assert.equal(changed.version, "V2");
  assert.notEqual(
    (
      await api.json(P + "reminder-scheme/save", {
        body: { ...row, reason: "过期版本" },
      })
    ).code,
    200,
  );
  ...
  assert.deepEqual(after.reminder, configured.reminder);
  const disabled = await api.ok(P + "reminder-scheme/status", {
    body: {
      id: row.id,
      version: changed.version,
      status: 0,
  ...
  assert.equal(disabled.status, 0);
  const other = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "停用提醒测试组" },
  });
  assert.notEqual(
    (
      await api.json(P + "project/group-save", {
        body: { ...other, reminder: { id: row.id } },
      })
    ).code,
    200,
  );
  const retained = await api.ok(P + "project/group-save", {
    body: { ...configured, description: "保留原提醒快照" },
  });
  assert.deepEqual(retained.reminder, configured.reminder);
});

test("editing survey wording preserves historical answer text and exported headers", async (t) => {
  const api = await start(t);
  await api.login();
  const original = await api.ok(P + "patient/survey-answer-detail", {
    query: { user_id: 5, template_id: 1 },
  });
  const form = await api.ok(P + "survey/detail", { query: { id: 1 } });
  const edited = structuredClone(form);
  edited.name = "新版问卷";
  edited.questions[0].title = "新版题干";
  edited.questions[0].options[0].label = "新版选项";
  await api.ok(P + "survey/save", { body: edited });
  const after = await api.ok(P + "patient/survey-answer-detail", {
    query: { user_id: 5, template_id: 1 },
  });
  assert.deepEqual(after, original);
  const current = await api.ok(P + "survey/detail", { query: { id: 1 } });
  assert.equal(current.questions[0].title, "新版题干");
  assert.equal(current.history.length, 1);
  assert.notEqual(
    (await api.json(P + "survey/save", { body: edited })).code,
    200,
  );
});
...
test("patient registration, treatment and dispensing preserve independent records", async (t) => {
  ...
  await api.login();
  const g = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "患者入组测试" },
  });
  const catalog = await api.ok(P + "project/catalog");
  const source = catalog.medication_schemes[0];
  await api.ok(P + "project/group-save", {
    body: {
      ...g,
      medication: {
  ...
  const p = await api.ok(P + "patient/save", { body });
  ...
  assert.notEqual(
    (
      await api.json(P + "patient/save", {
        body: { ...body, mobile: "13900000998", birth_date: "" },
      })
    ).code,
    200,
  );
  assert.notEqual(
    (
      await api.json(P + "patient/save", {
        body: { ...body, mobile: "13900000998", birth_date: "2027-01-01" },
      })
    ).code,
    200,
  );
  assert.notEqual(
    (
      await api.json(P + "patient/state", {
        body: {
          user_id: p.id,
          state: "治疗中",
  ...
  assert.notEqual(
    (
      await api.json(P + "patient/treatment", {
        body: {
          user_id: p.id,
          start_date: TODAY,
          treatment_days: 29,
    ...
    (
      await api.json(P + "patient/treatment", {
        body: {
          ...
          treatment_days: 30,
          reason: "尝试覆盖分组剂量",
          drugs: source.drugs.map((d, i) =>
            i ? d : { ...d, dose: String(Number(d.dose) + 1) },
          ),
        },
      })
    ).code,
  ...
  assert.equal(treatment.source_group_id, g.id);
  assert.deepEqual(
    treatment.drugs.map((d) => d.drug_id),
  ...
  const disp = await api.ok(P + "patient/dispense", {
    body: {
      ...
      issued_date: TODAY,
      reason: "实际发药",
      items: treatment.drugs.map((d) => ({ drug_id: d.drug_id, quantity: 30 })),
    },
  });
  assert.equal(disp.items.length, source.drugs.length);
  const before = await api.ok(P + "patient/management", {
    query: { user_id: p.id },
  });
  assert.notEqual(
    (
      await api.json(P + "patient/dispense", {
        body: {
          user_id: p.id,
          issued_date: TODAY,
          reason: "错误药品",
  ...
  const after = await api.ok(P + "patient/management", {
    query: { user_id: p.id },
  });
  assert.deepEqual(after, before);
  assert.equal(after.dispensings.length, 1);
  const group = await api.ok(P + "project/group-detail", {
    query: { project_id: 1, id: g.id },
  });
  assert.ok(group.participant_ids.includes(p.id));
  const filtered = await api.ok(P + "patient/index", {
    query: { keyword: body.mobile },
  });
  assert.equal(filtered.total, 1);
});

test("one-step onboarding is atomic and individual medication changes stay patient scoped", async (t) => {
  const api = await start(t);
  await api.login();
  const catalog = await api.ok(P + "project/catalog"),
    source = catalog.medication_schemes.find((s) => s.status === 1);
  const group = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "连续建档测试组" },
  });
  const configured = await api.ok(P + "project/group-save", {
    body: {
      ...group,
  ...
  const first = await api.ok(P + "patient/onboard", {
    body: {
      patient: patient("连续建档甲", "13900000701"),
      treatment: { start_date: TODAY, reason: "采用分组方案", adjusted: false },
      dispense: {
        issued_date: TODAY,
        reason: "首次发药",
        items: source.drugs.map((d) => ({ drug_id: d.drug_id, quantity: 20 })),
      },
    },
  });
  assert.equal(first.treatment.adjusted, false);
  assert.equal(first.treatment.treatment_days, 30);
  assert.equal(first.dispense.items.length, source.drugs.length);
  const changedDrug = {
    ...source.drugs[0],
    dose: String(Number(source.drugs[0].dose) + 0.5),
    times: "08:30",
  };
  const second = await api.ok(P + "patient/onboard", {
    body: {
      patient: patient("连续建档乙", "13900000702"),
      treatment: {
        start_date: TODAY,
        treatment_days: 20,
        reason: "根据患者情况调整",
  ...
  assert.equal(second.treatment.treatment_days, 20);
  assert.equal(second.treatment.drugs.length, 1);
  assert.ok(second.treatment.adjustment_summary.length >= 2);
  const firstView = await api.ok(P + "patient/management", {
      query: { user_id: first.patient.id },
    }),
    secondView = await api.ok(P + "patient/management", {
      query: { user_id: second.patient.id },
    });
  assert.equal(firstView.treatments.at(-1).drugs.length, source.drugs.length);
  assert.equal(
    secondView.treatments.at(-1).drugs[0].dose,
    Number(changedDrug.dose),
  );
  const groupAfter = await api.ok(P + "project/group-detail", {
    query: { project_id: 1, id: configured.id },
  });
  assert.deepEqual(groupAfter.medication.snapshot.drugs, source.drugs);
  assert.ok(
    groupAfter.participant_ids.includes(first.patient.id) &&
      groupAfter.participant_ids.includes(second.patient.id),
  );
  const beforeFailure = groupAfter.participant_ids.length;
  const failed = await api.json(P + "patient/onboard", {
    body: {
      patient: patient("连续建档回滚", "13900000703"),
      treatment: {
        start_date: TODAY,
        treatment_days: 30,
        reason: "错误调整",
  ...
  assert.equal(
    (await api.all(P + "patient/index", { keyword: "13900000703" })).length,
    0,
  );
  assert.equal(
    (
...
test("generated execution follows group dates and retains safety tasks when medication pauses", async (t) => {
  ...
  await api.login();
  const g = await api.ok(P + "project/group-create", {
    body: { project_id: 1, name: "任务生成测试" },
  });
  const source = (await api.ok(P + "project/catalog")).medication_schemes[0];
  await api.ok(P + "project/group-save", {
    body: {
      ...g,
      medication: {
  ...
  const p = await api.ok(P + "patient/save", {
    body: {
      name: "任务模拟患者",
      mobile: "13900000888",
      gender: 2,
  ...
  const paused = await api.all(P + "medication-plan/index", {
    scope: "all",
    user_id: p.id,
  });
  assert.equal(
    paused.filter((r) => r.status_text === "已暂停" && r.paused_by_state)
      .length,
    20,
  );
  assert.equal(
  ...
  const resumed = await api.all(P + "medication-plan/index", {
    scope: "all",
    user_id: p.id,
  });
  assert.equal(
  ...
  const exited = await api.all(P + "medication-plan/index", {
    scope: "all",
    user_id: p.id,
  });
...
test("report supplementation preserves originals and completes only its matching task", async (t) => {
  ...
  assert.notEqual(
    (
      await api.json(P + "report/create", {
        body: {
          user_id: 2,
          task_id: task.id,
          type: "检查",
  ...
  await api.ok(P + "report/supplement", {
    body: { id: report.id, note: "补传清晰图片", files: [uploaded.url] },
  });
  await api.ok(P + "report/review", {
    body: {
      id: report.id,
  ...
  const after = await api.ok(P + "report/detail", { query: { id: report.id } });
  assert.equal(after.versions.length, 2);
  assert.equal(after.history.length, 2);
  assert.equal(after.metrics[0].flag, "异常");
  const reportRows = workbookRows(
    Buffer.from(
      await (
        await api.raw(P + "research/export", {
          query: { kind: "reports", user_id: 1, keyword: "血常规" },
        })
      ).arrayBuffer(),
    ),
  );
  ...
  assert.notEqual(
    (
      await api.json(P + "report/review", {
        body: {
          id: report.id,
          status: "已核对",
          reason: "重复审核",
...
test("adverse assessment separates severity from seriousness and records manual contacts", async (t) => {
  ...
  const result = await api.ok(P + "adverse-reaction/assess", { body });
  assert.equal(result.assessment.serious, true);
  assert.equal(result.assessment.special_interest, true);
  assert.equal(result.severity, original.severity);
  assert.notEqual(
    (await api.json(P + "adverse-reaction/assess", { body })).code,
    200,
  );
  assert.notEqual(
    (
      await api.json(P + "adverse-reaction/assess", {
        body: { ...body, revision: 1, processing_status: "已处理" },
      })
    ).code,
    200,
  );
  await api.ok(P + "adverse-reaction/contact", {
    body: {
      id: 1,
      channel: "电话",
      result: "无人接听",
  ...
  const after = await api.ok(P + "adverse-reaction/assessment", {
    query: { id: 1 },
  });
...
test("feedback preserves per-day raw records and rejects conflicting symptom selections", async (t) => {
  ...
  const body = {
    ...
    date: TODAY,
    no_discomfort: false,
    symptoms: [{ name: "体重下降", change: "消失" }],
    note: "后台转录患者反馈",
    audio_url: "",
  };
  const row = await api.ok(P + "feedback/record", { body });
  assert.equal(row.source, "后台代录");
  assert.equal(row.symptoms[0].name, "体重下降");
  assert.equal(row.symptoms[0].change, "消失");
  assert.notEqual((await api.json(P + "feedback/record", { body })).code, 200);
  assert.notEqual(
    (
      await api.json(P + "feedback/record", {
        body: { ...body, user_id: 21, no_discomfort: true },
      })
    ).code,
    200,
  );
  const filtered = await api.ok(P + "feedback/index", {
    query: { user_id: 20, date: TODAY },
  });
  assert.equal(filtered.total, 1);
});
...
test("manual medication result is explicit and dashboard research metrics derive from rows", async (t) => {
  const api = await start(t);
  await api.login();
  const plans = await api.ok(P + "medication-plan/index", {
    query: { scope: "today", status: 0 },
  });
  const plan = plans.list[0];
  await api.ok(P + "medication-plan/record", {
    body: { id: plan.id, status: 2, reason: "电话核实患者明确未服" },
  });
  const filtered = await api.ok(P + "medication-plan/index", {
    query: { scope: "today", status: 2 },
  });
  assert.ok(filtered.list.some((r) => r.id === plan.id));
  const before = await api.ok(P + "dashboard/research");
  const task = await api.ok(P + "followup/create", {
    body: {
      user_id: 1,
      name: "过期提醒",
  ...
  assert.equal(after.tasks_completed, before.tasks_completed + 1);
});
...
test("new research exports are real XLSX and cover the filtered complete dataset", async (t) => {
  ...
  const emptyRows = workbookRows(
    Buffer.from(
      await (
        await api.raw(P + "research/export", {
          query: { kind: "tasks", keyword: "导出验证", group_id: 999999 },
        })
      ).arrayBuffer(),
    ),
  );
  ...
  const plan = (
      await api.ok(P + "medication-plan/index", {
        query: { scope: "all", size: 1, current: 1 },
      })
    ).list[0],
    planPatient = await api.ok(P + "patient/detail", {
      query: { user_id: plan.user_id },
    });
  const medicationRows = workbookRows(
    Buffer.from(
      await (
        await api.raw(P + "research/export", {
          query: {
            ...
            project_id: planPatient.project_id,
            group_id: planPatient.group_id,
  ...
  assert.equal(
    scopedDashboard.patients,
    (
      await api.ok(P + "patient/index", {
        query: {
          project_id: patient.project_id,
          group_id: patient.group_id,
          size: 100,
  ...
  const overview = await api.ok(P + "dashboard/overview", {
    query: {
      range: "today",
      project_id: patient.project_id,
      group_id: patient.group_id,
  ...
  assert.equal(overview.metrics.patient_total, scopedDashboard.patients);
  assert.notEqual(
    (await api.json(P + "research/export", { query: { kind: "invalid" } }))
      .code,
    200,
  );
});
...
test("daily feedback stores speech-converted text without audio fields or export columns", async (t) => {
  const api = await start(t);
  await api.login();
  const note = "语音转文字：今天咳嗽比昨天减轻，吃饭正常。";
  const body = {
    user_id: 24,
    date: TODAY,
    no_discomfort: false,
    symptoms: [{ name: "咳嗽", change: "减轻" }],
    note,
  };
  const saved = await api.ok(P + "feedback/record", { body });
  assert.equal(saved.note, note);
  assert.equal("audio_url" in saved, false);
  const rows = workbookRows(
    Buffer.from(
      await (
        await api.raw(P + "research/export", {
          query: { kind: "feedback", user_id: 24 },
        })
      ).arrayBuffer(),
    ),
  );
  assert.ok(rows.flat().includes(note));
  assert.ok(!rows[0].some((v) => v.includes("语音地址")));
  assert.notEqual(
    (
      await api.json(P + "feedback/record", {
        body: { ...body, user_id: 23, audio_url: "/api/mock-files/test" },
      })
    ).code,
    200,
  );
