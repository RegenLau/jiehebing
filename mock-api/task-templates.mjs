export function registerTaskTemplates({core,db,assert,find,page,clean,timestamp,nextId}) {
  ...
  core('POST','task-template/status',({body:b,admin})=>{const row=find(db.taskTemplates,b.id,'任务模板');assert(b.version===row.version,'模板已更新，请刷新后操作');assert([0,1].includes(b.status)&&b.status!==row.status,'请选择不同的有效状态');const reason=text(b.reason,'变更原因',300,true),before=snapshot(row);row.status=b.status;row.revision=(row.revision||1)+1;row.version=`V${row.revision}`;row.updated_at=timestamp();log(row,admin,before,reason);return row;});
}
