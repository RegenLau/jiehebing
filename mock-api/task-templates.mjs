export function registerTaskTemplates({core,db,assert,find,page,clean,timestamp,nextId}) {
  ...
  core('POST','task-template/save',({body:b,admin})=>{
    ...
    assert(types.includes(b.type),'请选择任务类型');
    const report_type=b.type==='检查'?text(b.report_type,'报告类型',100,true):'';
    assert(!db.taskTemplates.some(r=>r!==old&&r.name.toLowerCase()===name.toLowerCase()),'模板名称已存在');
    const row=old||{id:nextId(db.taskTemplates),status:1},before=old?snapshot(old):null;
    Object.assign(row,{name,type:b.type,report_type,description,requirements,revision:(old?.revision||0)+1,updated_at:timestamp()});row.version=`V${row.revision}`;log(row,admin,before,reason||'新增任务模板');if(!old)db.taskTemplates.push(row);return row;
  });
