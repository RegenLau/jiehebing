export function registerResearchExport({core,db,assert,clean,today,spreadsheet}) {
 core('GET','research/export',({query:q,res})=>{
  ...
  if(q.kind==='patients'){headers=['患者编号','姓名','手机号','性别','出生日期','研究分组','研究状态','入组基准日','患者端登录'];rows=db.patients.filter(p=>(!keyword||`${p.name} ${p.mobile} ${p.patient_code||''}`.includes(keyword))&&(!q.study_state||(p.study_state||'待启用')===q.study_state)).map(p=>[p.patient_code||p.id,p.name,p.mobile,p.gender_text,p.birth_date,p.group_name||'',p.study_state||'待启用',p.enroll_date,p.login_enabled?'可登录':'不可登录']);}
